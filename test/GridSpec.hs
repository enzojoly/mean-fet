...
sweepEnabled :: Bool
sweepEnabled = unsafePerformIO $ do
    v <- lookupEnv "ENCOUNTERS_SWEEP"
    return (v == Just "1")
{-# NOINLINE sweepEnabled #-}
...
twoRouteCheck :: TestTree
twoRouteCheck = testGroup "Reflecting low-mobility: matched-truncation route agreement (L=6)"
    [ testCase "GF-inversion and iterative PMFs agree at low mobility (shared truncation)" $ do
          let !pg  = encounterPMFReflect     6 1 0.05 0.20 1.0 (0, 0) (3, 3) 600
              !pit = encounterPMFReflectIter 6 1 0.05 0.20 1.0 (0, 0) (3, 3) 600
              !mg  = truncatedMean pg
              !mt  = truncatedMean pit
              !len = min (V.length pg) (V.length pit)
              !mx  = V.maximum (V.zipWith (\x y -> abs (x - y))
                                (V.take len pg) (V.take len pit))
          assertBool ("non-finite truncated means: " ++ show (mg, mt))
              (not (isNaN mg) && not (isNaN mt))
          assertBool ("GF-sum=" ++ show mg ++ " iter-sum=" ++ show mt
                      ++ " truncated means must agree (shared truncation cancels tail)")
              (abs (mg - mt) <= 1e-6 * mg + 1e-9)
          assertBool ("max|GF - iter| pointwise = " ++ show mx ++ " at low mobility")
              (mx < 1e-3)
    ]
...
sweepConfigTests
    | not sweepEnabled =
        testGroup "Sweep config (L=16 box) [skipped; set ENCOUNTERS_SWEEP=1]" []
    | otherwise = testGroup "Sweep config (L=16 box, starts (0,0)-(8,8))" $
...
anchorTests = testGroup "Second-round anchors"
    ...
    , testCase "gambler's-ruin: fundamental-matrix solve equals textbook interval mean" $ do
        let bigN = 12; q = 0.7 :: Double
            interiorN = bigN - 1
            fundamental :: [Double]
            fundamental =
                let aMat :: LA.Matrix Double
                    aMat = LA.build (interiorN, interiorN) $ \ri ci ->
                        let a = round ri :: Int; b = round ci :: Int
                        in if a == b then q
                           else if abs (a - b) == 1 then negate (q / 2)
                           else 0
                    rhs = LA.konst 1 interiorN :: LA.Vector Double
                in LA.toList (aMat LA.<\> rhs)
            textbook = [ fromIntegral (x * (bigN - x)) / q | x <- [1 .. interiorN] ]
            gap = maximum (zipWith (\a b -> abs (a - b)) fundamental textbook)
        assertBool ("fundamental-matrix solve vs textbook x(N-x)/q, max gap = " ++ show gap)
            (gap < 1e-9)
